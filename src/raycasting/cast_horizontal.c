...
{
	(*info)->ray->y = (((int)(*info)->player->y / GRID_SIZE) * GRID_SIZE) - 0.0001;
...
{
	(*info)->ray->y = (((*info)->player->y / GRID_SIZE) * GRID_SIZE) + GRID_SIZE;
	(*info)->ray->x = ((*info)->player->y - (*info)->ray->y)
						* (*info)->ray->tan + (*info)->player->x;
	(*info)->ray->y_offset = GRID_SIZE;
	(*info)->ray->x_offset = -(*info)->ray->y_offset * (*info)->ray->tan;
...
void	cast_horizontal(t_info **info)
{
	...
	if ((*info)->ray->sin > 0.001) // looking up
		iterate_up(info);
	if ((*info)->ray->sin < -0.001) // looking down
		iterate_down(info);
	if ((*info)->ray->angle == 0 || (*info)->ray->angle == 360) // looking straight left or right
